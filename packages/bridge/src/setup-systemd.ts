...
const SERVICE_NAME = "gotokens-bridge";
...
export function setupSystemd(opts: SetupOptions): void {
  ...
  const relayToken =
    opts.relayToken ?? process.env.BRIDGE_RELAY_TOKEN?.trim() ?? "";
  const relayRoomId =
    opts.relayRoomId ?? process.env.BRIDGE_RELAY_ROOM_ID?.trim() ?? "";
  const relayRoomSecret =
    opts.relayRoomSecret ?? process.env.BRIDGE_RELAY_ROOM_SECRET?.trim() ?? "";
...
`;
  ...
  try {
    ...
    console.log(
      "==> Service registered (start may have failed — check logs with: journalctl --user -u gotokens-bridge)",
    );
  }
