...
| `BRIDGE_RELAY_URL` | `wss://nqqlcknhdxys.sealoshzh.site` | 公開WebSocket relayのURL (未設定時はデフォルト公開relayを使用) |
| `BRIDGE_RELAY_TOKEN` | (なし) | relayが管理者トークンを要求する場合のみ設定 |
| `BRIDGE_RELAY_ROOM_ID` | `$HOME/.ccpocket/bridge-identity.json` | 安定したrelay room IDの上書き |
| `BRIDGE_RELAY_ROOM_SECRET` | `$HOME/.ccpocket/bridge-identity.json` | アプリ接続用の安定したrelay room secretの上書き |
...
ユーザー利便性のため、このプロジェクトでは独自実装のWebSocket relayを用意し、
公開環境にデプロイしている。これは一時的な外部サービスではなく、
ccpocket の通常アーキテクチャの一部として扱う。

- Bridge は `BRIDGE_RELAY_URL` 未設定時でもデフォルト公開relay
  (`wss://nqqlcknhdxys.sealoshzh.site`) を使用できる。
- Bridge は初回起動時に `$HOME/.ccpocket/bridge-identity.json` を生成し、
  relay room ID/secretを以後の再起動でも再利用する。これにより、同じPC上で
  Bridgeが起動している限り、スマホに保存済みのrelay接続URLは有効であり続ける。
  このファイルを削除すると、そのPCはrelay上で新しいマシンとして扱われる。
- relay はスマホから直接ローカルMacへ到達できない環境向けの接続経路で、
  Tailscale/LAN接続とは別の正式な接続方式。
- 接続・再接続の不具合調査では、必ず以下を分けて考える:
  1. スマホ ↔ 公開relay のWebSocket接続
  2. Bridge ↔ 公開relay の登録・再登録
  3. Bridgeローカルプロセス内の `SessionManager` / 実行中session状態
- Bridgeプロセス再起動後、relayへの再登録とスマホの再接続は成立しても、
  Bridgeプロセス内の実行中sessionはメモリ上の状態なので消える。
  その場合は旧bridge session idで `get_history` するのではなく、
  永続化されたClaude/Codex履歴から `resume_session` する必要がある。
